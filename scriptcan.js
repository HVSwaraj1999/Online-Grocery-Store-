...
$(document).ready(function () {
    ...
    var candyNameInput = $('#candyName');
    var lettersOnly = /^[A-Za-z]+$/;

    $('#candyForm').submit(function (event) {
        event.preventDefault();

        var userInput = candyNameInput.val();

        if (!lettersOnly.test(userInput)) {
            alert('Please enter a valid candy name (letters only).');
            return false;
        }

        $.ajax({
            url: 'get_inventorycan.php',
            type: 'GET',
            dataType: 'json',
            data: { candyName: userInput },
